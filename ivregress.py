...
def reg(X, y):
    ...
    return beta_hr  # 3 by 1 matrix of coefs
...
def _ivregress(X, Z, y, verbose=False):
    ...
    beta_2sls = reg(X_hat, y)  # X_hat has intercept (column of 1s)
    if verbose:
        print(beta_2sls)
    Pz = np.dot(tmp1, Z.T)
    if verbose:
...
def ivregress_2sls(df, y_var, regs, ev, inst, verbose=False):
    ...
    df_out = pd.DataFrame(
        data=np.concatenate([beta_2sls[:, None], std_err[:, None]], axis=1),
...
def ts2sls(df1, df2, y_var, regs, ev, inst, verbose=False):
    ...
    df_out = pd.DataFrame(
        data=np.concatenate([beta_ts2sls[:, None], std_err[:, None]], axis=1),
        columns=["Coef.", "Std. Err."],
        index=x_vars,
    )
